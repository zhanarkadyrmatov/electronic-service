    ...
    return (
        <div className={`${s.anketa} container`}>
            <form className={s.block} onSubmit={handleSubmit((data) => handleSubmitDara(data))}>
                ...
                <input {...register('lastName', { required: true })} placeholder="Тыныстанова, 54б, 2" />
                ...
                <input {...register('Five')} placeholder="Средний заработок" />
                <label htmlFor="">Фото паспорта(другая)</label>
                <button className={`${s.btn} center`}><LuFileInput size={15} /> Загрузить фото </button>
                <label htmlFor="">Согласие</label>
                <div className="flex" style={{gap:'8px',paddingBottom:'24px'}}>
                    <input style={{width:'15px',margin:'0'}} className={s.chek} type="checkbox" />
